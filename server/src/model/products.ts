...
//getById라는 함수를 정의, 매개변수로 문자열 id를 입력으로 받아, 
//products 배열에서 해당 id를 가진 제품 객체를 찾습니다.
export const getById = (id: string) => {
  //findIndex 메서드를 사용하여 id가 일치하는 제품의 인덱스를 찾습니다.
  //일치하는 제품이 없으면 -1을 반환합니다.
  const productIndex = products.findIndex((product) => product.id === id);
  
  //인덱스가 -1이 아닌 경우, 즉, 제품이 존재하면 해당 제품 객체를 반환합니다.
  if (products[productIndex]) {
    return products[productIndex];
  }

  //제품이 존재하지 않는 경우 null을 반환합니다.
  return null;
};

//deleteById라는 함수를 정의합니다. 
//이 함수는 문자열 id를 입력으로 받아, products 배열에서 해당 id를 가진 제품을 삭제합니다.
export const deleteById = (id: string) => {
  //findIndex 메서드를 사용하여 id가 일치하는 제품의 인덱스를 찾습니다.
  const productIndex = products.findIndex((product) => product.id === id);

  //splice 메서드를 사용하여 productIndex 위치의 제품을 배열에서 제거합니다.
  //두 번째 인자는 삭제할 항목의 개수
